...
public function sendPinCode()
{
    ...
    $user = new User();
    $user_info = $user->asObject()->where('email', $email)->first();

    // Save the PIN in the database (you can create a new table or add a column to the existing `users` table)
    $user->where('email', $email)->set(['pin_code' => $pin, 'pin_code_expiry' => Carbon::now()->addMinutes(10)])->update();

    // Send the PIN via email (or SMS if desired)
    $mail_data = [
        'pin' => $pin,
        'user' => $user_info
    ];
    ...
    $view = \Config\Services::renderer();
    $mail_body = $view->setVar('mail_data', $mail_data)->render('email-templates/pin-code-email-template');
    ...
    $mailConfig = [
        ...
        'mail_recipient_email' => $user_info->email,
        'mail_recipient_name' => $user_info->name,
        'mail_subject' => 'Your PIN Code for Password Reset',
        'mail_body' => $mail_body
    ];

    if (sendEmail($mailConfig)) {
        return redirect()->route('admin.forgot.form')->with('success', 'We have emailed your PIN code.');
    } else {
        return redirect()->route('admin.forgot.form')->with('fail', 'Something went wrong.');
    }
}
public function verifyPin($token)
{
    ...
    $user = new User();
    $user_info = $user->asObject()->where('pin_code', $token)->first();

    if (!$user_info) {
        return redirect()->route('admin.forgot.form')->with('fail', 'Invalid PIN code.');
    }

    return view('backend/pages/auth/reset_with_pin', [
...
public function verifyPinHandler($token)
{
    $isValid = $this->validate([
        'new_password' => [
            'rules' => 'required|min_length[8]|max_length[15]|is_password_strong[new_password]',
            'errors' => [
                ...
                'max_length' => 'New Password must not exceed 15 characters.',
    ...
    // Clear the PIN code after successful reset
    $user->where('email', $user_info->email)->set(['pin_code' => null])->update();

    return redirect()->route('admin.login.form')->with('success', 'Your password has been updated successfully.');
}
